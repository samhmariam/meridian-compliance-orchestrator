...
- **Economic (Latency):** End-to-end orchestration latency (p95).
  - *Baseline:* No pre-existing automated baseline exists. *Baseline collection plan:* End-to-end wall-clock latency (graph start to audit-close node completion) will be captured from Azure Monitor (Log Analytics / Application Insights) traces during the same D03 integration test run; p95 across 50 synthetic events will be recorded before any performance optimisation is attempted. *Target:* < 45 seconds (excluding human approval pause).
